...
test("account recovery action appears only for existing locked documents", async () => {
  ...
  try {
    ...
    const recovery = page.getByRole("button", { name: "ロックされた教材の最新データを端末に保存", exact: true });
    ...
    await account.click(); await expect(recovery).toBeVisible(); await account.click();
    await app.evaluate(() => { (globalThis as typeof globalThis & { lockedCount: number }).lockedCount = 0; });
    await account.click(); await expect(recovery).toHaveCount(0);
